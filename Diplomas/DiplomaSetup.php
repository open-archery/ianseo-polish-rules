 ...
 */
function pl_diploma_ensure_tables(): void
{
...
function pl_diploma_get_config($tourId)
{
    ...
    $Rs = safe_r_sql("SELECT * FROM PLDiplomaConfig WHERE PlDcTournament = " . (int) $tourId);
    if (safe_num_rows($Rs) > 0) {
        $row = safe_fetch($Rs);
        $defaults['CompetitionName'] = $row->PlDcCompetitionName;
        $defaults['Dates'] = $row->PlDcDates;
        $defaults['Location'] = $row->PlDcLocation;
        $defaults['PlaceFrom'] = (int) ($row->PlDcPlaceFrom);
        $defaults['PlaceTo'] = (int) ($row->PlDcPlaceTo);
 ...
 */
function pl_diploma_save_config($tourId, $data): void
{
    $tourId = (int) $tourId;
    ...
    if ($exists) {
        ...
        safe_w_sql(
            "INSERT INTO PLDiplomaConfig (PlDcTournament, PlDcCompetitionName, PlDcDates, PlDcLocation, PlDcPlaceFrom, PlDcPlaceTo, PlDcBodyText, PlDcHeadJudge, PlDcOrganizer) VALUES ("
            . $tourId . ", "
...
{
    $texts = [];
    $Rs = safe_r_sql("SELECT PlDeEventCode, PlDeCustomText FROM PLDiplomaEventText WHERE PlDeTournament = " . (int) $tourId);
    if (safe_num_rows($Rs) > 0) {
 ...
 */
function pl_diploma_save_event_text($tourId, $eventCode, $text): void
{
    $tourId = (int) $tourId;
