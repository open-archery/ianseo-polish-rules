<?php
// PL override of Common/pdf/chunks/DivClasIndividual.inc.php
// Adds "Nr lic." (bib/EnCode) column between category and country/club.
...
if(count($rankData['sections']))
{
	...
	foreach($rankData['sections'] as $section)
	{
		...
		pl_writeGroupHeaderPrnIndividual($pdf, $section['meta'], $DistSize, $AddSize, $section['meta']['numDist'], $rankData['meta']['double'], false, $hideGolds);
		...
		foreach($section['items'] as $item)
		{
			...
			{
				$pdf->AddPage();
				pl_writeGroupHeaderPrnIndividual($pdf, $section['meta'], $DistSize, $AddSize, $section['meta']['numDist'], $rankData['meta']['double'], true, $hideGolds);
			}
			pl_writeDataRowPrnIndividual($pdf, $item, $DistSize, $AddSize, $section['meta']['numDist'], $rankData['meta']['double'], ($PdfData->family=='Snapshot' ? $section['meta']['snapDistance']: 0), $hideGolds);
...
function pl_writeDataRowPrnIndividual($pdf, $item, $distSize, $addSize, $distances, $double, $snapDistance, $HideGolds=false)
{
...
function pl_writeGroupHeaderPrnIndividual($pdf, $section, $distSize, $addSize, $distances, $double, $follows=false, $HideGolds=false)
{
