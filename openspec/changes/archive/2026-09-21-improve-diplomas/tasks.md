...
- [x] 1.1 In `Fun_Diploma.php`, swap `CONCAT(Entries.EnFirstName, ' ', Entries.EnName)` to
...
- [x] 1.2 Apply the same swap in `pl_diploma_get_all_athletes()` and `pl_diploma_get_athlete()`;
...
- [x] 1.3 Add `gotchas.md` entry documenting that ianseo's `EnFirstName` column holds the surname
      and `EnName` holds the given name, in the same commit as 1.1/1.2.

## 2. Date/location line and config-exists gating

- [x] 2.1 In `PLDiplomaPdf.php`, change `printDiploma()`'s bottom line to build from whichever of
      ...
      (Extracted `pl_diploma_build_date_location_line()` into `Fun_Diploma.php`, covered by
      `testBuildDateLocationLine`'s 5-case data provider.)
- [x] 2.2 In `DiplomaSetup.php`, add a `ConfigExists` boolean to `pl_diploma_get_config()`'s
...
- [x] 2.3 In `DiplomaConfig.php`, gate the session pre-fill block on `!$config['ConfigExists']`
      ...
      (Gated the whole pre-fill block, including CompetitionName, per design.md Decision 8.)

## 3. Custom diploma bugs

- [x] 3.1 In `PrnCustomDipl.php`, fix the category-text lookup to read
      ...
      (Fixed by routing through `pl_diploma_resolve_category_line()` from task 4.2, which reads
      `['customText']` correctly — covered by that function's own test coverage.)
- [x] 3.2 In `PrnCustomDipl.php`, compute `titleText` the same way `PrnIndividualDipl.php`/
      ...
      (`pl_diploma_build_title()` already has full coverage; PrnCustomDipl.php's own wiring is
      exercised by task 6.2's manual check since it's a print-script integration, not new pure logic.)

## 4. Category/division phrase composition

- [x] 4.1 In `DiplomaSetup.php`, add `pl_diploma_get_category_defaults($rawEventCode)` returning
...
- [x] 4.2 In `Fun_Diploma.php`, add `pl_diploma_resolve_category_line($rawEventCode, $evType,
...
- [x] 4.3 In `DiplomaSetup.php`'s `pl_diploma_ensure_tables()`, add `PlDeCategoryName VARCHAR(100)
...
- [x] 4.4 Extend `pl_diploma_get_event_texts()`'s and `pl_diploma_save_event_text()`'s signatures
      ...
      the two new fields.
- [x] 4.5 In `PLDiplomaPdf.php`, rename `printDiploma()`'s `$classText` parameter to
      ...
      (Per manual review during 6.2: also added a forced line break before "w kategorii" — see
      design.md Decision 4 — since the composed line wraps onto a second line in every real
      combination anyway, and bumped the competition-name line from 14pt to 16pt.)
- [x] 4.6 In `PrnIndividualDipl.php`, `PrnTeamDipl.php`, and `PrnCustomDipl.php`, replace the
...
- [x] 4.7 In `DiplomaConfig.php`, add "Nazwa kategorii" and "Rodzaj łuku" inputs to the per-event
...
- [x] 5.1 In `DiplomaSetup.php`, change `pl_diploma_get_config()`'s `$defaults['PlaceTo']` from
      ...
      `PlaceTo === 8`.
      (Done alongside 2.2 since both touch the same `$defaults` array/function.)

## 6. Manual verification

- [x] 6.1 In a running ianseo instance with a `TourLocRule='PL'` tournament: open
      ...
      (Verified live; also surfaced and fixed the `Diplomas.php` summary showing a bare
      "Data: | Miejsce: |" for blank values — now shows "-".)
- [x] 6.2 Generate one individual, one team, and one mixed-team diploma batch (covering a Senior
      ...
      (Verified live; also added a forced line break before "w kategorii" and bumped the
      competition-name font size per review of the rendered diplomas.)
