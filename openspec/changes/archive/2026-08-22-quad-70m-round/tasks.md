...
- [x] 2.1 Add `$isDouble = (isset($subRuleName) && $subRuleName === 'Poland-4x70m');` near the top of the script (after `$TourType = 3;`) — `$SubRule` itself is a raw dropdown position from the real UI, not the rule name; `$subRuleName` is the actual string, shared into scope via `GetSetupFile()`'s `require_once`.
- [x] 2.2 Add the local `pl_double_legs($legs, $isDouble)` helper (see design.md Decision 2) that groups legs by meters value and relabels each group sequentially (`70m-1..70m-4`, `40m-1,40m-2,20m-1,20m-2`) when `$isDouble` is true, and returns `$legs` unchanged otherwise.
- [x] 2.3 Wrap the distances argument of every `CreateDistanceNew()` call (RM, RW, RU24M, RU24W, RU21M, RU21W, RU18M, RU18W, R50M, R50W, RU15M, RU15W, `C%`, `B%` — 14 calls total) in `pl_double_legs(..., $isDouble)`.
...
- [x] 3.1 Run `grep -n "CreateDistanceNew(" Setup_3_PL.php` and confirm every match's distances argument is wrapped in `pl_double_legs(`. — all 14 confirmed wrapped.
