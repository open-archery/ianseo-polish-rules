...
- `LiveQualification.php` - page shell. `CheckTourSession(true)`, `checkFullACL(AclQualification, '', AclReadOnly)`. Renders session/distance `<select>` controls (sessions via existing `GetSessions('Q')` helper, same as `SelectSession.php`; distances via a `DistanceInformation` query scoped to the chosen session) and an empty table container. Includes the polling JS. No `<form>` action beyond the selection controls themselves - selection changes trigger a re-fetch, not a submit/reload.
