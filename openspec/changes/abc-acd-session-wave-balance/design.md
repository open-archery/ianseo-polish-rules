...
`pl_abc_acd_session_wave_tally(int $tourId, int $sesOrder, string $excludeEvent): array` runs a read-only query joining `Qualifications`+`Entries` (same shape as `pl_abc_acd_load_athletes`), filtered to `EnTournament`, `QuSession`, `QuTarget!=0`, and `CONCAT(TRIM(EnDivision),TRIM(EnClass)) != excludeEvent` (exact match, not a `LIKE` pattern, since `excludeEvent` may itself contain `%`/`_` wildcards that the page permits in `Event`). Groups by `EnCountry`, counts `QuLetter IN ('A','B')` as wave1 and `QuLetter IN ('C','D')` as wave2. Returns `club_code => ['wave1' => int, 'wave2' => int]`.
...
- **Clubs 2+ (overflow, "first column that fits"):** same `needA` sign decides the whole search order — `A → C → B → D` when wave2-heavy/no-history, `C → A → D → B` when wave1-heavy. `B` and `D` are independently assignable pools (odd-boss `B` slots vs even-boss `D` slots within the same target range), so they carry the same wave1/wave2 bias as `A`/`C`. The slot-by-slot fallback (triggered when no single column has enough remaining room) walks this same club-specific order, not a fixed `A→C→B→D`, so overflow athletes get the bias too — while still tracking bosses already used by the club so two of its athletes never share one.
