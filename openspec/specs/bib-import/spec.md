...
| Age class (`EnAgeClass`) | Resolved in Step 3 (blank if unresolved) |
| Competition class (`EnClass`) | Same value as `EnAgeClass` — ianseo uses two columns; BibImport always sets both to the resolved `Classes.ClId` |
